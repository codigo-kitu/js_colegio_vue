...
import {procesarTodosDatosData} from '@/lib/proceso/nota/infrastructure/data/NotaData';
...
const getTodosDatos = async () => { 			
	...
	//ocultarLoader();			
	emit(Constantes.UPDATE_DATOS_VIEW,notas.value);
};
...
const procesarTodosDatos = async () => {
	...
	const data:NotaReturnView = await procesarTodosDatosData(props.module,props.controller,pagination1.value);
...
const siguientes = async () => {
	...
	if(notas.value != null && notas.value.length > 0) {
		pagination1.value.skip = pagination1.value.skip + pagination1.value.limit;
	}
...
<div id="div_nota_form_buscar" class="div_buscar_general">
		...
		<div id="div_FK_Idalumno" class="tab_item">
			<form id="nota_FK_Idalumno_form_buscar" class="form_buscar_general">
		...
		<div id="div_FK_Iddocente" class="tab_item">
			<form id="nota_FK_Iddocente_form_buscar" class="form_buscar_general">
		...
		<div id="div_FK_Idmateria" class="tab_item">
			<form id="nota_FK_Idmateria_form_buscar" class="form_buscar_general">
