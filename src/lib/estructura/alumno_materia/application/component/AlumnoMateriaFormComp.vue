...
const props = defineProps({
	...
	alumno_materia_prop: {type: AlumnoMateria, default: new AlumnoMateria()}
});
...
//------------------ GENERAL --------------------
let title = ref('Formulario Alumno Materia');
...
const handleAction_ClickTableRow = (alumno_materia1:AlumnoMateria) => {
	...
	if(Constantes.IS_DEVELOPING == true) {
		text_id_aux.value = alumno_materia1.id.toString();
	}
				
	alumno_materia.value.id = alumno_materia1.id;
	alumno_materia.value.created_at = alumno_materia1.created_at;
	
	alumno_materia.value.updated_at = alumno_materia1.updated_at;
	alumno_materia.value.id_alumno = alumno_materia1.id_alumno;
	alumno_materia.value.id_materia = alumno_materia1.id_materia;
};
...
const nuevoDatos = async () => {
	...
	const data_json:AlumnoMateriaReturnView = await nuevoDatosData(props.module,props.controller,alumno_materia.value);
...
const actualizarDatos = async () => {	
	...
	const data_json:AlumnoMateriaReturnView = await actualizarDatosData(props.module,props.controller,alumno_materia.value);
...
const eliminarDatos = async () => {
	...
	const data_json:AlumnoMateriaReturnView = await eliminarDatosData(props.module,props.controller,alumno_materia.value);
...
const getFks = async () => {
	...
	const data:AlumnoMateriaReturnView = await getFksData(props.module,props.controller);
