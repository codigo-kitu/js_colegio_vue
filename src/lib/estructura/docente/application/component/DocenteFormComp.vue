...
import {nuevoDatosData,
		actualizarDatosData,
		eliminarDatosData
		} from '@/lib/estructura/docente/infrastructure/data/DocenteData';
...
const props = defineProps({
	...
	docente_prop: {type: Docente, default: new Docente()}
});
...
//------------------ GENERAL --------------------
let title = ref('Formulario Docente');
...
const handleAction_ClickTableRow = (docente1:Docente) => {
	...
	if(Constantes.IS_DEVELOPING == true) {
		text_id_aux.value = docente1.id.toString();
	}
				
	docente.value.id = docente1.id;
	docente.value.created_at = docente1.created_at;
	
	docente.value.updated_at = docente1.updated_at;
	docente.value.nombre = docente1.nombre;
	docente.value.apellido = docente1.apellido;
	docente.value.fecha_nacimiento = docente1.fecha_nacimiento;
	
	docente.value.anios_experiencia = docente1.anios_experiencia;
	docente.value.nota_evaluacion = docente1.nota_evaluacion;
};
...
const nuevoDatos = async () => {
	...
	const data_json:DocenteReturnView = await nuevoDatosData(props.module,props.controller,docente.value);
...
const actualizarDatos = async () => {	
	...
	const data_json:DocenteReturnView = await actualizarDatosData(props.module,props.controller,docente.value);
...
const eliminarDatos = async () => {
	...
	const data_json:DocenteReturnView = await eliminarDatosData(props.module,props.controller,docente.value);
		...
		<div class="modal_form_general_body">
			...
			<div id="div_docente_form_general">
				...
				<form id="docente_form_general" 
					...
					<label for="anios_experiencia" class="">Anios Experiencia</label>
					<input  type="text" 
							id="anios_experiencia" name="anios_experiencia"
							class="" placeholder="Anios Experiencia"
							v-model="docente.anios_experiencia" />
					
					<label for="nota_evaluacion" class="">Nota Evaluacion</label>
					<input  type="text" 
							id="nota_evaluacion" name="nota_evaluacion"
							class="" placeholder="Nota Evaluacion"
